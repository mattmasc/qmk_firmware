...
static void place_delimiter(void) {
    if (IS_MODIFIER_KEYCODE(xcase_delimiter)) {
        ...
        if (record->event.pressed) {
            ...
            if (xcase_state == XCASE_ON) {
                ...
                // don't increment distance to last delim if negative
                    ...
                    if (distance_to_last_delim == 0 && (IS_MODIFIER_KEYCODE(xcase_delimiter))) {
                        place_delimiter();
                    }
                    ++distance_to_last_delim;
                    delimiters_count = 0;
            ...
            if (terminate_case_modes(keycode, record)) {
                bool was_caps_word_on = caps_word_on;
                disable_caps_word();
                disable_xcase();
                if (was_caps_word_on && keycode == CM_CAMEL) {
